...
ssh <vps> 'grep -r "proxy_read_timeout\|proxy_connect_timeout" /etc/nginx/ 2>/dev/null'
# Should show 3600 (1 hour) — adequate for 30s long-poll sync

# Synapse resource health
ssh <vps> 'systemctl is-active matrix-synapse && free -h | head -2 && uptime'

# Synapse log for sync-related errors
ssh <vps> 'grep -i "sync\|timeout\|warn\|error" /var/log/matrix-synapse/homeserver.log 2>/dev/null | tail -20'
```
...
for i in 1 2 3; do
    T=$(curl -sk -o /dev/null -w '%{time_total}' -H "Host: matrix.gloriosa.space" \
         --max-time 10 https://<REDACTED>/_matrix/client/versions 2>/dev/null)
...
```bash
# Replace <REDACTED> with VPS-HK's overlay IP
echo "<REDACTED> matrix.gloriosa.space" | sudo tee -a /etc/hosts
```
