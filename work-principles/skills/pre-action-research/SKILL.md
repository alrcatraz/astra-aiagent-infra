...
- [ ] **是否需要做 Environment Baseline（环境基线）检查？**
  - 适用场景：SSH 到新机器、排查服务异常、部署前验证、系统健康审计、维护窗口前评估
  - 逐项收集以下信息（非破坏性、纯只读命令，在所有 Linux 上安全执行）：
  - [ ] **systemd 服务状态** — `systemctl list-units --type=service --state=running`（只看 running，避免噪声）；如有异常关注 `systemctl list-units --type=service --state=failed`
  - [ ] **监听端口** — `ss -tlnp`（TCP listening ports + 进程名）。注意 0.0.0.0 vs 127.0.0.1 的暴露面差异；高风险端口（22, 5432, 6379, 27017, 3306, 9200, 9090, 3000, 8080）如有 0.0.0.0 绑定需特别记录
  - [ ] **Docker 容器状态** — `docker ps -a`（含已停止容器），关注 restart policy 和状态。同时 `docker stats --no-stream 2>/dev/null` 看资源占用（CPU/内存）
  - [ ] **挂载点与磁盘** — `df -h` 看磁盘使用率（>80% 标黄，>90% 标红），`mount | grep -E '^/dev|^tmpfs|nfs|cifs'` 看关键挂载
  - [ ] **crontab 条目** — 检查 `/var/spool/cron/crontabs/` 和 `/etc/crontab`，以及 `crontab -l`（当前用户）；关注高频率定时任务是否可能冲突
  - [ ] **网络配置** — `ip addr`（接口与 IP）、`ip route`（路由表）、`ip neigh`（ARP 表，看可见的局域网设备数）。如有 `resolvectl status` 也收集 DNS 配置
  - [ ] **内核/发行版信息** — `uname -a` 和 `cat /etc/os-release`
  - [ ] 将收集到的基线信息写入会话上下文（不需要持久化，除非是 `server-health-audit` 等需要审计留痕的场景）
