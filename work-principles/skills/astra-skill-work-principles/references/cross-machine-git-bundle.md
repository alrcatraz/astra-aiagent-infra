...
>
> 你的本地配置（<device> 地址、SSH 别名、密码）→ `work-closure-check/references/git-bundle-local-config.md`。
...
### Step 3: Apply bundles on <device>
...
6. **<device> password may differ from the GPG passphrase.** These are stored as separate secrets — see the local config file for specific values.
